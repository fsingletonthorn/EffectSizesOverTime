...
mod <- rma.mv(z, V = SEz^2, random = ~  1 | journal / factor(article), mods = year, data = datMeta[sample(1:nrow(datMeta), size = 1000, replace = F),])



# All data analysis
datComplete <- dat[!is.na(dat$z)& !(dat$z == Inf) &!is.na(dat$SEz) & !(dat$SEz == Inf),]
modAll <- rma.mv(z, V = SEz^2, random = ~  1 | journal / factor(article), mods = year, data = datComplete[sample(1:nrow(datComplete), size = 1000, replace = F),])
...
modLargest <- rma.mv(z, V = SEz^2, random = ~ 1 | journal / article, mods = (year - mean(year)), data = datMetaLargest[sample(1:nrow(datMetaLargest), size = 1000, replace = F),])
