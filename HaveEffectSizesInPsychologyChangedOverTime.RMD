...
All analyses were tested and developed on a subset of 0.01% of the dataset before being pre-registered (see https://osf.io/e3kr7/). 
...
mod <- rma.mv(z, V = SEz^2, random = ~  factor(article) | journal, mods = (year - mean(year)), data = datMeta[sample(1:nrow(datMeta), size = 1000, replace = F),])


# All data analysis
datComplete <- dat[!is.na(dat$z)& !(dat$z == Inf) &!is.na(dat$SEz) & !(dat$SEz == Inf),]
modAll <- rma.mv(z, V = SEz^2, random = ~  factor(article) | journal, mods = (year - mean(year)), data = datComplete[sample(1:nrow(datComplete), size = 1000, replace = F),])
...
modLargest <- rma.mv(z, V = SEz^2, random = ~ 1 | journal, mods = (year - mean(year)), data = datMetaLargest[sample(1:nrow(datMetaLargest), size = 1000, replace = F),])
...
modFirst <- rma.mv(z, V = SEz^2, random = ~  1 | journal, mods = (year - mean(year)), data = datMetaFirst[sample(1:nrow(datMetaFirst), size = 1000, replace = F),])
...
This analysis shows that there has been effectivly no change in the average effect sizes reported in these journals over time, with an estimated decrease per year of `r mod$b[2]` (using all data) in Fisher z score units. This corresponds to a correlation coefficent change per year of just `r transf.ztor(mod$b[2])`. The two models using the first reported statistical test and the largest statistical test in each article argree, showing a negligable decrease over time. 
...
ggplot(data = datMeta, aes(y = z, x = year, group = year))+ geom_boxplot(alpha = .1) 
...
plot(dat %>% 
  group_by(year) %>%
  summarise(mean_z = mean(z, na.rm = T)))
