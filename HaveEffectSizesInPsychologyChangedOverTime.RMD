...
The multilevel-meta-regression includes random effects for individual tests $u_{id}$, articles $u_{article}$ and journals $u_{journal}$, and includes year of publication of each article as a fixed effect ($\gamma_1Year$). This analysis was performed using nlme {Pinheiro, 2018 #1027} in R version 3.5.1 {R Development Core Team, 2018 #314}. 

As a check on whether the exclusion of F statistics with $df_1$ of greater than one and $\chi^2$ analyses is likely to change the results, this multilevel model was reperformed including all of the statistical tests for which correlations could be estimated and estimating all standard errors as above (i.e.,  $\sqrt{1/(df_2 - 5)}$ for F tests and $\sqrt{1/(df - 5)}$ for $\chi^2$ tests), a total of n = `r nrow(datComplete)` effects after excluding all invalid results (i.e., analyses where it was not possible to estimate a standard error or correlation using the above methods). This analysis lead to estimates of the change in effect sizes over time which are practically identical (i.e., which differ by < `r -0.002 # calculated below see "differenceAllMeta"`). The results below present the results excluding those studies.
...
niceModSum <- data_frame(Estimate = c(mod$coefficients$fixed, rep(NA,4)), 
           ...
           "Random effects" = c(rep(NA, 2), 
                                ...
                                paste0("QE(", nrow(datMeta), ") = ", round(sum(residuals(mod, type="pearson")^2),2), 
                                       ", p = < .001") 
                                ))
           VarCorr(mod)
