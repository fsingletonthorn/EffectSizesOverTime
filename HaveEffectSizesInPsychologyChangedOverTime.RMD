...
## Analysis 
Standard errors were estimated as $\sqrt{1/(n - 3)}$.
...
dat$vaidSE <- !(dat$Statistic=="Chi2" | (dat$Statistic=="F"& dat$df2 > 2) | (dat$Statistic == "Z"))
...
datMeta <- dat[dat$vaidSE,]

rma.mv(z, V = SEz^2, data = datMeta)

dat$z
random = (X1 | Source | journal)
...
### Conversion 
All statistical tests extracted were transformed into correlation coefficents using the methods reported in {Open Science Collaboration, 2015 #611}. 

t statistics were converted using:
...
Where $F_{obs}$ is the observed F statistic and $df_1$ is the degrees of freedom of the numerator and $df_2$ is degrees of freedon of the denominator.  
